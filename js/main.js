...
const agregar = (e) => {
  ...
  if (!n || !a || !act || !t) return mostrarMensaje('Completá todos los campos', 'warning');
  ...
  if (ocupados >= t.cupo) return mostrarMensaje('No hay cupos disponibles para ese turno', 'danger');
  reservas.push({ nombre: n, apellido: a, actividad: act, turno: t.horario });
  guardar();
  render();
  nombre.value = apellido.value = '';
  actividad.value = turno.value = '';
